...
void AMapGenerator::Tick(float DeltaTime) {
    /* // should iterate stored units and update vision
    for (auto &Elem : PlayerUnits)
    {
        FVector PlayerLocation = Elem->GetActorLocation();
    }*/
...
AMapGenerator* AMapGenerator::GetMapGenerator()
{
    return Cast<AMapGenerator>(this);
}
...
void AMapGenerator::BeginPlay() {
  ...
  if (TileMapComponent) {
    InitTerrainData();
    GenerateTerrainData();
    //TileMapComponent->SetCollisionEnabled(ECollisionEnabled::Type::QueryAndPhysics);
